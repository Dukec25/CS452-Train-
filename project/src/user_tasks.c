...
{
	irq_debug(DEBUG_TASK, "enter %s", "name_server_task");
	uint32 tid = MyTid();
	irq_debug(DEBUG_TASK, "starting name_server_task tid = %d", tid); 
	name_server_start();
...
void clock_server_task(){
	irq_debug(DEBUG_UART_IRQ, "enter %s", "clock_server_task");
	uint32 tid = MyTid();
	irq_debug(DEBUG_UART_IRQ, "starting clock_server_task tid = %d", tid); 
	clock_server_start();
	irq_debug(DEBUG_UART_IRQ, "tid =%d exiting", tid);
	Exit();
}

void clock_server_notifier(){
	irq_debug(DEBUG_UART_IRQ, "enter %s", "clock_server_notifier");
	Delivery request; 
	Clock_server_message reply_message;
	vint clock_server_tid = WhoIs("CLOCK_SERVER");
	while(1){
		/*irq_debug(DEBUG_UART_IRQ, "before enter %s", "awaitEvent");*/
		request.data = AwaitEvent(TIMER3_RDY, -1); // evtType = here should be clock update event;
		/*irq_debug(DEBUG_UART_IRQ, "after enter awaitEvent, send to clock_server_tid = %d", clock_server_tid);*/
		request.type = CLOCK_NOTIFIER;
		Send(clock_server_tid, &request, sizeof(request), &reply_message, sizeof(reply_message));
		/*irq_debug(SUBMISSION, "after enter %s", "clock notify");*/
	}
}

void uart1_rcv_server(){
	uint32 tid = MyTid();
	irq_debug(DEBUG_UART_IRQ, "starting io_server_task tid = %d", tid); 
...
void uart1_rcv_notifier(){
	irq_debug(DEBUG_UART_IRQ, "enter = %s", "rcv_notifier");
	...
	while(1){
		...
		irq_debug(DEBUG_UART_IRQ, "receive_notifer get awaked= %s", "");
	}
}
...
void uart2_rcv_notifier(){
	...
	while(1){
		...
		Send(io_server_id, &request, sizeof(request), &reply_msg, sizeof(reply_msg) );
		irq_debug(DEBUG_UART_IRQ, "uart2 receive_notifer get awaked", "");
	}
}

void uart1_xmit_notifier(){
	irq_debug(DEBUG_UART_IRQ, "enter = %s", "xmit_notifier");
	...
	while(1) {
		Send(io_server_id, &request, sizeof(request), &reply_msg, sizeof(reply_msg));
		irq_debug(DEBUG_UART_IRQ, "received reply_msg.data = %d", reply_msg.data);
		AwaitEvent(XMIT_UART1_RDY, reply_msg.data);
		irq_debug(DEBUG_UART_IRQ, "wake up from %s", "XMIT_RDY");
		/*Delay(5);*/
...
void uart1_xmit_enable(){
	...
	irq_debug(DEBUG_TASK, "created taskId = %d", tid);
...
void uart2_xmit_enable(){
	...
	irq_debug(DEBUG_UART_IRQ, "created taskId = %d", tid);
...
void first_task()
{
	...
	irq_debug(DEBUG_K4, "created taskId = %d", tid);
